...
// ── Ukrainian name spell-correction ──
...
function correctWord(word: string, dict: string[]): string {
  ...
  if (exact) return exact; // fix wrong case
  let best = word, bestDist = 3;
  for (const d of dict) {
    if (d[0].toLowerCase() !== lw[0]) continue; // must start same letter
...
function correctNameSpelling(text: string): string {
  const words = text.trim().split(/\s+/);
  return words.map((w, i) => {
    if (!w) return w;
    if (i === 0) return w; // surname — skip (too many possible values)
