import { chromium, devices } from "playwright";
import { readFileSync } from "node:fs";

const CANDIDATE_URLS = [
  "http://127.0.0.1:8080",
  "http://localhost:8080",
  "http://127.0.0.1:5173",
  "http://localhost:5173",
];

async function detectBaseUrl() {
  for (const url of CANDIDATE_URLS) {
    try {
      const res = await fetch(url, { method: "GET" });
      if (res.ok) return url;
    } catch {
      // try next
    }
  }
  throw new Error("Dev server not reachable on known URLs (8080/5173)");
}

async function ensureOperational(page) {
  const operational = page.getByRole("button", { name: /Операційна/i });
  if (await operational.count()) {
    await operational.first().click({ timeout: 3000 }).catch(() => {});
  }
}

function parseEnvFile(filePath) {
  try {
    const raw = readFileSync(filePath, "utf8");
    const out = {};
    for (const line of raw.split(/\r?\n/)) {
      const trimmed = line.trim();
      if (!trimmed || trimmed.startsWith("#") || !trimmed.includes("=")) continue;
      const idx = trimmed.indexOf("=");
      const key = trimmed.slice(0, idx).trim();
      const val = trimmed.slice(idx + 1).trim().replace(/^"|"$/g, "");
      out[key] = val;
    }
    return out;
  } catch {
    return {};
  }
}

function getSupabaseConfig() {
  const envLocal = { ...parseEnvFile(".env.local"), ...parseEnvFile("proctocare-ua-dash/.env.local") };
  const envBase = { ...parseEnvFile(".env"), ...parseEnvFile("proctocare-ua-dash/.env") };
  const env = { ...envBase, ...envLocal, ...process.env };

  const active = (env.VITE_SUPABASE_ENV || "test").toLowerCase();
  const isProd = active === "prod";
  const url = isProd ? env.VITE_SUPABASE_PROD_URL : env.VITE_SUPABASE_TEST_URL;
  const key = isProd ? env.VITE_SUPABASE_PROD_ANON_KEY : env.VITE_SUPABASE_TEST_ANON_KEY;

  if (!url || !key) {
    throw new Error("Supabase config missing in .env.local");
  }

  return { url, key, active };
}

async function supabaseInsert({ url, key, table, payload }) {
  const res = await fetch(`${url}/rest/v1/${table}`, {
    method: "POST",
    headers: {
      apikey: key,
      Authorization: `Bearer ${key}`,
      "Content-Type": "application/json",
      Prefer: "return=representation",
    },
    body: JSON.stringify(payload),
  });

  if (!res.ok) {
    throw new Error(`Supabase insert failed (${table}): ${res.status} ${await res.text()}`);
  }
}

async function supabasePatch({ url, key, table, matchColumn, matchValue, payload }) {
  const res = await fetch(`${url}/rest/v1/${table}?${matchColumn}=eq.${encodeURIComponent(matchValue)}`, {
    method: "PATCH",
    headers: {
      apikey: key,
      Authorization: `Bearer ${key}`,
      "Content-Type": "application/json",
      Prefer: "return=representation",
    },
    body: JSON.stringify(payload),
  });

  if (!res.ok) {
    throw new Error(`Supabase patch failed (${table}): ${res.status} ${await res.text()}`);
  }
}

async function seedPatientVisit(config, patientId, fullName, dateIso) {
  const [surname, firstName, patronymic] = fullName.split(/\s+/);

  await supabaseInsert({
    ...config,
    table: "patients",
    payload: [{
      id: patientId,
      name: `${surname} ${firstName}`,
      patronymic,
      phone: "+380671110000",
      birth_date: "01.01.1985",
      is_test: true,
    }],
  });

  await supabaseInsert({
    ...config,
    table: "visits",
    payload: [{
      id: patientId,
      patient_id: patientId,
      visit_date: dateIso,
      visit_time: "17:00",
      procedure: "Колоноскопія",
      status: "planning",
      ai_summary: "e2e",
      from_form: true,
      is_test: true,
    }],
  });
}

async function openPatientCard(page, uniqueSurname) {
  const patientCardName = page.locator(`text=${uniqueSurname}`).first();
  await patientCardName.waitFor({ timeout: 30000 });
  await patientCardName.click();
  await page.locator("h2", { hasText: uniqueSurname }).first().waitFor({ timeout: 15000 });

  const cardTab = page.getByRole("button", { name: /^Карта$/ });
  if (await cardTab.count()) {
    await cardTab.first().click().catch(() => {});
  }
}

async function applyRealtimeUpdates(config, patientId, expected) {
  await Promise.all([
    supabasePatch({
      ...config,
      table: "patients",
      matchColumn: "id",
      matchValue: patientId,
      payload: {
        birth_date: expected.birthDate,
        phone: expected.phone,
        allergies: expected.allergies,
        diagnosis: expected.diagnosis,
      },
    }),
    supabasePatch({
      ...config,
      table: "visits",
      matchColumn: "id",
      matchValue: patientId,
      payload: {
        notes: expected.notes,
        procedure: expected.serviceAdded,
        status: "progress",
      },
    }),
  ]);
}

async function waitFieldChecks(page, expected) {
  const results = {};

  await page.waitForFunction((birthDate) => {
    const el = document.querySelector('input[placeholder="ДД.ММ.РРРР"]');
    return !!el && el.value === birthDate;
  }, expected.birthDate, { timeout: 30000 });
  results.birthDate = true;

  await page.waitForFunction((phone) => {
    const el = document.querySelector('input[placeholder="+380671234567"]');
    return !!el && el.value.replace(/\s+/g, "") === phone;
  }, expected.phone, { timeout: 30000 });
  results.phone = true;

  await page.waitForFunction((value) => document.body.innerText.includes(value), expected.allergies, { timeout: 30000 });
  results.allergies = true;

  await page.waitForFunction((value) => document.body.innerText.includes(value), expected.diagnosis, { timeout: 30000 });
  results.diagnosis = true;

  await page.waitForFunction((value) => document.body.innerText.includes(value), expected.notes, { timeout: 30000 });
  results.notes = true;

  await page.waitForFunction((value) => document.body.innerText.includes(value), expected.serviceAdded, { timeout: 30000 });
  results.procedure = true;

  return results;
}

async function run() {
  const baseUrl = await detectBaseUrl();
  const supabaseConfig = getSupabaseConfig();
  const token = Date.now();
  const surname = `E2E${token}`;
  const fullName = `${surname} Іван Іванович`;
  const patientId = `e2e-${token}`;
  const today = new Date();
  const dateIso = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;

  const expected = {
    birthDate: "12.05.1988",
    phone: "+380671112233",
    allergies: `Алергія-${token}`,
    diagnosis: `Діагноз-${token}`,
    notes: `Нотатки-${token}`,
    serviceAdded: "Гастроскопія",
  };

  const browser = await chromium.launch({ channel: "chrome", headless: true });
  const desktopContext = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const mobileContext = await browser.newContext({ ...devices["Pixel 7"] });

  const desktop = await desktopContext.newPage();
  const mobile = await mobileContext.newPage();

  const report = {
    ok: false,
    baseUrl,
    supabaseEnv: supabaseConfig.active,
    createdPatient: fullName,
    checks: {},
  };

  try {
    await Promise.all([
      desktop.goto(baseUrl, { waitUntil: "domcontentloaded" }),
      mobile.goto(baseUrl, { waitUntil: "domcontentloaded" }),
    ]);

    await Promise.all([
      desktop.getByText("ProctoCare").first().waitFor({ timeout: 15000 }),
      mobile.getByText("ProctoCare").first().waitFor({ timeout: 15000 }),
    ]);

    await Promise.all([ensureOperational(desktop), ensureOperational(mobile)]);

    await seedPatientVisit(supabaseConfig, patientId, fullName, dateIso);
    await Promise.all([desktop.reload(), mobile.reload()]);
    await Promise.all([ensureOperational(desktop), ensureOperational(mobile)]);

    await Promise.all([
      openPatientCard(desktop, surname),
      openPatientCard(mobile, surname),
    ]);

    await applyRealtimeUpdates(supabaseConfig, patientId, expected);

    const mobileChecks = await waitFieldChecks(mobile, expected);
    const desktopChecks = await waitFieldChecks(desktop, expected);

    report.checks = {
      mobile: mobileChecks,
      desktop: desktopChecks,
    };
    report.ok = [...Object.values(mobileChecks), ...Object.values(desktopChecks)].every(Boolean);
  } finally {
    await desktopContext.close();
    await mobileContext.close();
    await browser.close();
  }

  console.log(JSON.stringify(report, null, 2));
}

run().catch((error) => {
  console.error("verify-all-fields-sync failed:", error);
  process.exit(1);
});
